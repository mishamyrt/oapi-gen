...
Start with the [quick start](../README.md#quick-start) for a complete working app.
This page covers the generated API, additional response types, and schema rules.

- [Generated packages](#generated-packages)
- [Responses and errors](#responses-and-errors)
- [Security](#security)
- [File responses](#file-responses)
- [Streaming responses](#streaming-responses-openapi-32)
- [Response validation](#response-validation)
- [Supported OpenAPI features](#supported-openapi-features)
- [Upgrading generated code](#upgrading-generated-code)
...
`from app.http.generated.contracts import ListCats` and
`from app.http.generated.contracts.cats import ListCats` import the same class.
`create_router` combines the groups in the original path/method order. Shared
models remain in `models.py`.
...
Operation and field descriptions appear in generated contract docstrings, so
they remain available in your IDE while you implement handlers.
...
bytes. Codecs are created once. For non-streaming responses, multipart uploads close
when the handler finishes, including after validation failures or exceptions.
Authorization follows the declared security alternatives and scopes.
...
Include the `contracts` and `routes` subpackages when distributing generated code.
For dependency injection, see the [Dishka integration](../packages/oapi-gen-dishka/README.md).

## Responses and errors

The following snippets assume the named operations, statuses, bodies, and headers
are declared in your spec. Import their namespaces with
`from app.http.generated import contracts`.
...
as another operation's `Ok`, raises `TypeError`.

### Response headers

Declared response headers are typed fields on each variant:
...
headers keep their comma-separated serialization.
...
`oapi-gen` supports the following subset:
...
Regenerate the package after upgrading the generator and update your handlers
...
Update handler protocol imports from `*Api` to `*Handler`.
...
For cookie arrays, replace raw strings in `set_cookie=[...]` with
`contracts.Cookie(...)` objects.
